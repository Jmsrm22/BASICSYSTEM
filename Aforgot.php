<?php
    ...
    if($_SESSION['forgot'] == 'valid'){
        echo "<script>window.location.href = 'http://localhost/finals/Achangepass.php'</script>";
    }
     ...
     if(isset($_POST['submit'])){
        ...
        $code = $_POST['code'];
         ...
         if($code == $PresCode){
            ...
            $sql = "SELECT * FROM `admin` WHERE `email` = '$email' AND `fname` = '$fname' AND `lname` = '$lname'";
            $result = mysqli_query($con,$sql);
                ...
                $row = mysqli_fetch_assoc($result);
                    ...
                    if($row['fname'] === $fname && $row['lname'] === $lname && $row['email'] === $email){
                        ...
                        $_SESSION['forgot'] = 'valid';
                        header("refresh: 0; Achangepass.php");
                        echo '<script>alert("You can Change your Username and Password.")</script>';
                        exit();

                    }else{
                         $_SESSION['forgot'] = 'invalid';
                        header("refresh: 0; Aforgot.php");
...
<body class="bg-image">
    ...
    <div class="row">
            <h1 class="head bg-success bg-opacity-75 text-white mb-5" style="text-align:center;">COSTODA NAIC</h1>
        </div>
    ...
    <div class="container-fluid">
        ...
        <div class="col 12 text-center text-white mt-3">
            <p>Click to Go <a href="Alogin.php" class="link-offset-light">Log In Page.</a></p>
        </div>
